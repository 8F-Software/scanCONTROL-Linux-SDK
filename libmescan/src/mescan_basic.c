                                ...
                                guint32 *m0, guint32 *m1)
{
    ...
    for (gint32 i = 0; i < resolution * 64; i += 64) {
        ...
        if (x != NULL) {
            x[i / 64] = calc_x_coo(data[i + irefl + 4], data[i + irefl + 5], scaling, 0);
        }
        if (z != NULL) {
            z[i / 64] = calc_z_coo(data[i + irefl + 6], data[i + irefl + 7], scaling, offset, 0);
        }
        if (m0 != NULL) {
                                     ...
                                     gushort *threshold, double *x, double *z, guint32 *m0, guint32 *m1)
{
    ...
    for (gint32 i = d_start; i < point_ct * d_width - d_start - 16; i += d_width) {
        ...
        if (x != NULL && d_start <= irefl + 4 && d_width >= (irefl + 6 - d_start)) {
            x[i / d_width] =
                calc_x_coo(data[i + irefl + 4], data[i + irefl + 5], scaling, 0);
        }
        if (z != NULL && d_start <= irefl + 6 && d_width >= (irefl + 8 - d_start)) {
            z[i / d_width] =
                calc_z_coo(data[i + irefl + 6], data[i + irefl + 7], scaling, offset, 0);
        }
...
gint32 get_llt_type_by_name(const char *full_model_name, TScannerType *scanner_type)
{
    ...
    if (scanner_type != NULL) {
        ...
        char model_type[3] = {0};
        strncpy(model_type, &(full_model_name[12]), 2);
        model_type[2] = '\0';
        ...
        strncpy(measuring_range, &(full_model_name[17]), 4);
        measuring_range[3] = '\0';
        ...
        } else if (strcmp(measuring_range, "200") == 0) {
            *scanner_type -= 997;
        } else {
            *scanner_type = StandardType;
            return GENERAL_FUNCTION_DEVICE_NAME_NOT_SUPPORTED;
...
gint32 get_scaling_and_offset_by_type(TScannerType scanner_type, double *scaling, double *offset)
{
    ...
    if (scanner_type == scanCONTROL29xx_10) {
        ...
        tmp_scaling = 0.002;
        tmp_offset = 125.0;
    } else if (scanner_type == scanCONTROL30xx_200) {
        tmp_scaling = 0.007;
        tmp_offset = 320.0;
    } else if (scanner_type == scanCONTROL30xx_100) {
        tmp_scaling = 0.005;
        tmp_offset = 270.0;
    } else {
        return GENERAL_FUNCTION_DEVICE_NAME_NOT_SUPPORTED;
